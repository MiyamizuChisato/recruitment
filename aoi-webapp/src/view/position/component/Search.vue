...
import { pageSearch } from '../../../api/position.js';
...
const key = ref('');
const search = async () => {
    const { code, data } = await pageSearch(key.value, page.value.current);
    onApiSuccess(code, () => {
        page.value = data;
    });
};
const getCard = async () => {
    const { code, data } = await pageSearch(route.params.key, page.value.current);
    onApiSuccess(code, () => {
        page.value = data;
    });
};
